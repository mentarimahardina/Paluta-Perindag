...
<div class="card">
  <div class="card-header">
    <div class="row">
      <div class="col-md-10">
        <h4 class="m-0 font-weight-bold text-primary">Daftar event</h4>
      </div>
      <div class="col-md-2" style="text-align: end;">
        <button type="submit" class="btn btn-primary" href="#" data-toggle="modal"
          data-target="#event-add">Tambah event</button>
      </div>
    </div>
        ...
        foreach ($data as $key => $item) {
          ...
          <tr>
            ...
            <td style="text-align: center;">
              <a data-toggle="modal" id="eventinfo" data-target="#event-info" data-id="<?= $item['id'] ?>"
                data-post_title="<?= $item['post_title'] ?>" data-post_content="<?= str_replace('"', "'", $item['post_content']) ?>"
              ...
              <a data-toggle="modal" id="eventedit" data-target="#event-edit" data-id="<?= $item['id'] ?>"
                data-post_title="<?= $item['post_title'] ?>" data-post_content="<?= str_replace('"', "'", $item['post_content']) ?>"
                data-post_image="<?= $item['post_image'] ?>" data-post_author="<?= $item['post_author'] ?>"
                data-post_categories="<?= $item['post_categories'] ?>" data-post_type="<?= $item['post_type'] ?>"
                data-post_status="<?= $item['post_status'] ?>" data-post_visibility="<?= $item['post_visibility'] ?>"
                ...
                <a data-toggle="modal" id="eventdel" data-target="#event-delete" data-id="<?= $item['id'] ?>"
